  ...
  const handlePlaylistClick = async (playlistId) => {
    ...
    const handleAddEditor=async(editor)=>{
      try{
        ...
        const response = await axios.post('http://localhost:3001/Users/addEditorToPlaylist', {
          emailId:creatoremail,
          playlistId: selectedPlaylist,
          editorEmail: editor.emailID,
        });
    ...
    const handleAddEitor = async (editor) => {
      try {
        const selectedPlaylistId = playlistId; // Assuming you're selecting the first playlist for simplicity
        console.log(`bbbbbbbbbbbbbb${selectedPlaylistId}`)
        const username=userState.user?.firstName;  
        ;
        ...
        if (response.data.message === 'Editor added successfully') {
